const XML_BLOCK_RE = /^<([a-zA-Z][a-zA-Z0-9_-]*)>\s*\n([\s\S]*?)\n<\/\1>\s*$/gm;
const MERMAID_BLOCK_RE = /^```mermaid\n([\s\S]*?)```$/gm;
const FRONTMATTER_RE = /^---\n([\s\S]*?)\n---(?:\n|$)/;

/** Encode content for safe embedding in HTML attributes. */
...
export function preprocessContent(raw: string): Processed {
  ...
  body = body.replace(XML_BLOCK_RE, (_match, tagName: string, content: string) => {
    ...
    const i = idx++;
    return `<div data-type="xmlBlock" data-tag-name="${tagName}" data-content="${encodeHtmlEntities(content)}" data-index="${i}"></div>`;
  });
  ...
  body = body.replace(MERMAID_BLOCK_RE, (_match, code: string) => {
    ...
    const i = mIdx++;
    return `<div data-type="mermaidBlock" data-code="${encodeHtmlEntities(code.trimEnd())}" data-index="${i}"></div>`;
  });
